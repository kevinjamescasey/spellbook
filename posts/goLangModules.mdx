...
You can try creating a module to contain the `main` 
``` sh
...
...and having the `go mod tidy` command add the dependency delcaration, but the package will not be found.
``` sh
>go mod tidy
...
That line from the language spec impies that every package needs to be in a subdirectory of its module's root directory.
...
#### Package split up into mutliple files 😐
...
Notice that the mame (`x`) in the package declaration and the name (`y`) in the import do not have to match and neither one of them match the name of the directory that contains the package's files (`packageInModule`).
