...
The model I chose appears to be a base model that just extends the text you provide as if it is trying to finish you sentence. The ones with "instruct" in the name, such as `meta-llama/Llama-3.2-1B-Instruct`, appear to have instruction fine tuning that makes that respond as if they're another person having a conversation with you. 
