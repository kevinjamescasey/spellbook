...
| container image        | file archive that contains other files required to run piece of software                         | [hello-world](https://hub.docker.com/_/hello-world)                |
| container registry     | a place to store, look up, and retrieve container images                                         | Docker Hub, AWS ECR,                                               |
| container engine       | software that runs containers (based on images) on a runtime. Usually has a CLI and image cache. | Docker Engine, CRI-O, Railcar, RKT, LXC, Podman, Colima            |
| container runtime      | software component that container engines delegate container running to                          | runc, containerd, Incus, CRI-O, crun, railcar, katacontainers, MRC |
| container host         | machine with OS, virtual machine with OS, or another container where the containers will run     |                                                                    |
| container orchestrator | tool that starts, stops, and monitors containers often distributed across multiple hosts          | Kubernetes, ECS, Docker Compose Swarm, whatever PaaSs use          |
|                        |                                                                                                  |                                                                    |
...
``` shell
brew install docker colima minikube kubectl podman
```
...
The Homebrew `docker` package installs the CLI, but not the engine. If you want to use the engine you'll have to install Docker Desktop.
...
Has the same CLI interface as `docker`. It runs Fedora CoreOS on a VM using different VM technology depending on which host OS it is on. Does not run a deamon server like Docker does.
...
+ The `docker` CLI uses different container engines according to configuration
  + `docker context ls` lets you see the available contexts and which one is currently in use
...
+ `kubectl config get-contexts` shows which clusters exist and which one it is using
  + `kubectl config use-context CONTEXT-NAME`to switch contexts/clusters
  + `kubectl get ns` to view namespaces on a context's cluster
  + `kubectl config set-context --current --namespace NAMESPACE-NAME` to set the namespace to use in the context
  + `kubectl --context CONTEXT-NAME -n NAMESPACE-NAME COMMAND` to specify context and namespace for each command
...
Incus documentation provides [comparisons of application containers, system containers, and virtual machines](https://linuxcontainers.org/incus/docs/main/explanation/containers_and_vms/). 
